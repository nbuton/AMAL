...
def main():
    ...
    mafonction = Linear1Simple()
    ctx = Context()
    output = mafonction.forward(ctx, x, w, b)
    gradient_originel = torch.ones(1, requires_grad=True, dtype=torch.float64)
    mafonction_grad = mafonction.backward(ctx, gradient_originel)
    ...
    pred = Linear1.forward(ctx,x,w,b)
    true = torch.randn(batch,requires_grad=True,dtype=torch.float64)
    assert torch.autograd.gradcheck(MSE.apply,(true,pred))
    ...
    for n_iter in range(nb_iter):
        ...
        pred = Linear1.forward(ctx_linear,x,w,b)
        erreur = MSE.forward(ctx_mse,y,pred)
        #On calcul les backwards
        gradient_originel = torch.ones(1, requires_grad=True, dtype=torch.float64)
        grad_y,grad_yhat = MSE.backward(ctx_mse,gradient_originel)
        grad_x,grad_w,grad_b = Linear1.backward(ctx_linear,grad_yhat)
        w = w - (epsilon*(grad_w/x.shape[0]))
        b = b - (epsilon*(grad_b/x.shape[0]))
        loss_train = np.mean(MSE.forward(ctx_mse,train_target,Linear1.forward(ctx_linear,train,w,b)).detach().numpy())
        writer.add_scalar('Loss_stochastique/train', loss_train, n_iter)
        loss_test = np.mean(MSE.forward(ctx_mse,test_target,Linear1.forward(ctx_linear,test,w,b)).detach().numpy())
    ...
    for n_iter in range(nb_iter):
        ...
        w = w - (epsilon*(grad_w/x.shape[0]))
        b = b - (epsilon*(grad_b/x.shape[0]))
        loss_train = torch.mean(MSE.forward(ctx_mse,train_target,Linear1.forward(ctx_linear,train,w,b)))#.detach().numpy()
        writer.add_scalar('Loss_batch/train', loss_train, n_iter)
        loss_test = torch.mean(MSE.forward(ctx_mse,test_target,Linear1.forward(ctx_linear,test,w,b)))#.detach().numpy()
        writer.add_scalar('Loss_batch/test', loss_test, n_iter)

    #Descente par mini-batch
    nb_iter = 100
    taille_batch = 32
    epsilon = 1e-6
    data_in = train.shape[1]
    ...
    ctx_mse = Context()
    for n_iter in range(nb_iter):
        choix = np.random.randint(0,len(train),taille_batch)
        #On rajoute la taille de batch de 1
        x = train[choix]
        y = train_target[choix]
        pred = Linear1.forward(ctx_linear,x,w,b)
        erreur = MSE.forward(ctx_mse,y,pred)
        #On calcul les backwards
        gradient_originel = torch.ones(1, requires_grad=True, dtype=torch.float64)
        ...
        b = b - (epsilon*(grad_b/x.shape[0]))
        loss_train = torch.mean(MSE.forward(ctx_mse,train_target,Linear1.forward(ctx_linear,train,w,b)))#.detach().numpy()
        writer.add_scalar('Loss_mini_batch/train', loss_train, n_iter)
        loss_test = torch.mean(MSE.forward(ctx_mse,test_target,Linear1.forward(ctx_linear,test,w,b)))#.detach().numpy()
        writer.add_scalar('Loss_mini_batch/test', loss_test, n_iter)
